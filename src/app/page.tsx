...
export default function Home() {
    return (
        <main className="min-h-screen flex flex-col items-center">
            ...
            {/* Hero Section */}
            <section className="relative w-full max-w-6xl mx-auto px-6 pt-24 pb-32 flex flex-col items-center text-center z-10">
                ...
                <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-10 leading-relaxed">
                    You've seen me on Reddit as <span className="text-pink-400 font-semibold hover:underline cursor-pointer">u/professionalgur9997</span>. Step into my private world for exclusive behind-the-scenes content you won't find anywhere else.
                </p>
                ...
                <div className="mt-24 grid grid-cols-3 gap-4 md:gap-8 w-full max-w-3xl glass-panel p-6 md:p-8 hover-lift">
                    <div className="flex flex-col items-center">
                        <span className="text-3xl md:text-4xl font-bold text-white mb-2">Exclusive</span>
                        <span className="text-xs md:text-sm text-gray-400 uppercase tracking-wider">Content</span>
                    </div>
                    <div className="flex flex-col items-center border-x border-white/10">
                        <span className="text-3xl md:text-4xl font-bold text-white mb-2">24/7</span>
                        <span className="text-xs md:text-sm text-gray-400 uppercase tracking-wider">Chat</span>
                    </div>
                    <div className="flex flex-col items-center">
                        <span className="text-3xl md:text-4xl font-bold text-white mb-2">Private</span>
                        <span className="text-xs md:text-sm text-gray-400 uppercase tracking-wider">Live Streams</span>
                    </div>
                </div>
            </section>

            {/* Content Gallery Section */}
            <section id="content" className="w-full max-w-6xl mx-auto px-6 py-24 z-10">
                <div className="flex justify-between items-end mb-12">
                    <div>
                        <h2 className="text-3xl md:text-4xl font-bold mb-4">Latest Insights</h2>
                        <p className="text-gray-400 max-w-lg">A sneak peek into the private feed. Unlock to view the full experience.</p>
                    </div>
                    <button className="hidden md:block px-6 py-2 rounded-full border border-pink-500/30 text-pink-400 text-sm font-medium hover:bg-pink-500/10 transition-colors">
                        View All Content
                    </button>
                </div>
            ...
            <section id="pricing" className="w-full max-w-6xl mx-auto px-6 py-24 mb-24 z-10 flex flex-col items-center">
                <h2 className="text-4xl md:text-5xl font-bold text-center mb-6">Choose Your Access</h2>
                <p className="text-xl text-gray-400 text-center max-w-2xl mb-16">Join the inner circle to get unrestricted access to the entire private gallery and member-only perks.</p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl relative">

                    {/* Background Glow for pricing cards */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-pink-600/20 blur-[100px] rounded-full -z-10" />

                    {/* Standard Tier */}
                    <div className="glass-panel p-8 md:p-12 flex flex-col hover-lift">
                        <h3 className="text-2xl font-bold mb-2 text-white">VIP Club</h3>
                        <p className="text-gray-400 mb-8 h-12">Perfect for getting started and seeing what goes on behind closed doors.</p>
                        <div className="flex items-baseline mb-8">
                            <span className="text-4xl font-bold text-white">Monthly Access</span>
                        </div>
                        ...
                        <ul className="space-y-4 mb-10 flex-1">
                            {['Full access to premium gallery', 'Weekly exclusive updates', 'Direct messaging access', 'Cancel anytime'].map((feature, i) => (
                                <li key={i} className="flex items-center text-gray-300">
                                    <svg className="w-5 h-5 text-pink-500 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                                    {feature}
                                </li>
                            ))}
                        </ul>

                        <button className="w-full py-4 rounded-xl border border-white/20 hover:bg-white/10 text-white font-bold transition-all text-center">
                            Subscribe to VIP
                        </button>
                    </div>

                    {/* Lifetime Tier */}
                    <div className="glass-panel p-8 md:p-12 flex flex-col relative overflow-hidden transform md:scale-105 border-pink-500/50 hover-lift hover-glow shadow-pink-500/10 shadow-2xl">
                        {/* Best value tag */}
                        ...
                        <p className="text-gray-400 mb-8 h-12">The ultimate experience. Get lifetime access and personal interaction.</p>
                        <div className="flex items-baseline mb-8">
                            <span className="text-4xl font-bold text-white">Lifetime Access</span>
                        </div>
                        ...
                        <button className="w-full py-4 rounded-xl gradient-bg text-white font-bold transition-transform hover:scale-[1.02] shadow-lg shadow-pink-500/25">
                            Unlock All Access
                        </button>
                    </div>
                </div>
            ...
            <footer className="w-full border-t border-white/10 py-12 px-6 mt-auto flex flex-col items-center glass-panel rounded-none z-10">
                ...
                <div className="flex gap-6 mb-8">
                    {['Reddit', 'Instagram', 'Twitter', 'Terms', 'Privacy'].map((item) => (
